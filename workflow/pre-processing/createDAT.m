...
basename = [path '/' baseInput]; % base name of the tif files
fid = fopen(baseOutput, 'w');    % open the output file to write in

%If mesh =1, adds mesh 2 slices before outlet
mesh=0;

%%%%%%%%%%%%%%%%%%%%% INLET SLICE %%%%%%%%%%%%%%%%%%%%%%%%
% fname = [basename num2str(1, '%0.4i') '.tif'];
% fnameu = [basename num2str(2, '%0.4i') '.tif'];
fname = [basename num2str(1, '%0.4i') ];
fnameu = [basename num2str(2, '%0.4i') ];
BB=imread(fname ,'tif');
