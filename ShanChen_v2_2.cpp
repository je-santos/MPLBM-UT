// Modified to calculate velocity /density (momentum) from force increasing (f1 and f2) with single-phase flow included in runs 1 and 2
...
void PorousMediaSetup(MultiBlockLattice3D<T, DESCRIPTOR>& lattice_fluid1,
    ...
    T rhoNoFluid, T rho_fluid1, T rho_fluid2, T Gads_f1_s1, T Gads_f1_s2, T force_fluid1, T force_fluid2, T nx1f1, T nx2f1, T ny1f1, T ny2f1, T nz1f1, T nz2f1, T nx1f2, T nx2f2, T ny1f2, T ny2f2, T nz1f2, T nz2f2, plint runs, T& Force1, T& Force2)
{
	...
	T Force1 =	force_fluid1;
	T Force2 =	0;
	
	pcout << "Force fluid 1 = " << Force1 << endl;
    pcout << "Force fluid 2 = " << Force2 << endl;
	
    setExternalVector(lattice_fluid1, lattice_fluid1.getBoundingBox(), DESCRIPTOR<T>::ExternalField::forceBeginsAt, Array<T, 3>(0., Force1, 0.));
    setExternalVector(lattice_fluid2, lattice_fluid2.getBoundingBox(), DESCRIPTOR<T>::ExternalField::forceBeginsAt, Array<T, 3>(0., Force2, 0.));
    ...
    initializeAtEquilibrium(lattice_fluid2, Box3D(1, nx-1, 1, ny-1, 1, nz-1), rho_fluid2, zeroVelocity);
    initializeAtEquilibrium(lattice_fluid1, Box3D(1, nx-1, 1, ny-1, 1, nz-1), rhoNoFluid, zeroVelocity);
	
	T Force1 =	0;
	T Force2 =	force_fluid2;
	...
	T Force1 =	force_fluid1*(runs-2);
	T Force2 =	force_fluid2*(runs-2);
...
int main(int argc, char* argv[])
{
	...
	const plint runnum = ((rho_fluid1_max - rho_fluid1_min) / rho_fluid1_step)+ 10;
	//const plint runnum = 8;	
    T rho_fluid1[runnum] ;
    T rho_fluid2[runnum];
	//T mu1[runnum];
	//T mu2[runnum];
	...
	*/
	...
	for (plint readnum = 1; readnum <= runnum; ++readnum) {
        rho_fluid1[readnum] = rho_fluid1_min;
    }
	for (plint readnum = 1; readnum <= runnum; ++readnum) {
    ...
    lattice_fluid2.periodicity().toggle(0, true);
    lattice_fluid1.periodicity().toggle(0, true);
	...
	pcout << "Simulation runs = " << runnum << endl;
	...
	//for (plint Run = 1; Run <= 2; ++Run) {
    for (plint runs = 1; runs <= runnum; ++runs) { // Loop simulations with varying saturation

        PorousMediaSetup(lattice_fluid1, lattice_fluid2, geometry, rhoNoFluid, rho_fluid1[1], rho_fluid2[1], Gads_f1_s1, Gads_f1_s2, force_fluid1, force_fluid2, nx1f1, nx2f1, ny1f1, ny2f1, nz1f1, nz2f1, nx1f2, nx2f2, ny1f2, ny2f2, nz1f2, nz2f2, runs, Force1, Force2);
