...
int main(int argc, char* argv[])
{
	...
	const plint runnum = (((rho_fluid2_max - rho_fluid2_min) / rho_fluid2_step)*2)+3;	
    T rho_fluid1[runnum] ;
    T rho_fluid2[runnum];
	//T mu1[runnum];
	//T mu2[runnum];
	...
	*/
	...
	for (plint readnum = 3; readnum <= runnum; ++readnum) {
		if (readnum % 2 != 0) {
        rho_fluid2[readnum] = rho_fluid2_max - (readnum-3)* rho_fluid2_step*0.5;
		}
		else {
