...
for sample_num=[26111 ]
    sample_num
    load(['G:\My Drive\Documents\ClassesUT\PhD Research\models\slipshalenet\data_ying\Porosity_' num2str(sample_num) '_DL\p_20MPa\Uz_P_20MPa_1ds.mat'])
    
    dx=0.5e-9;
    %dx=5.7e-6;
    sim_size=size(uz_s1,3);
    ...
    Uz_P=uz_s1*usurf_PL/(1e-8/accelz_L);
    k = mean(Uz_P,'all')*mu_P/pressgra_P
    porosity = sum(Uz_P~=0,'all')/numel(Uz_P)*100
    mean_ap = sum(Uz_P~=0,'all')/256^2
end