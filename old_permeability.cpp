...
void writeGifs(MultiBlockLattice3D<T,DESCRIPTOR>& lattice, plint iter)
{
   ...
   // Write velocity-norm at z=nz/2.
    imageWriter.writeScaledGif(createFileName("uz_half", iter, 6),
              *computeVelocityNorm(lattice, Box3D(0,nx-1, 0,ny-1, nz/2,nz/2)),
              imSize, imSize );
}
...
int main(int argc, char **argv)
{
    ...
    double oldAvgE = 0.001;
    double relE = 0.000001;
    ...
    //util::ValueTracer<T> converge(1.0,3000.0,1.0e-4);
    ...
    for (;iT<maxT; ++iT) {
        ...
        if (iT % 20 == 0 && iT>0) {
            ...
            pcout << "-----------------"  << std::endl;
            pcout << "AvgEnergy: "<< getStoredAverageEnergy(lattice) <<  std::endl;
            relE= std::fabs(oldAvgE-getStoredAverageEnergy(lattice))/oldAvgE;
            pcout << "Relative Diff: "<< relE <<  std::endl;
            oldAvgE=getStoredAverageEnergy(lattice);
            pcout << "MaxVel: "<< getStoredMaxVelocity(lattice) <<  std::endl;
            pcout << "AvgRho: "<< getStoredAverageDensity(lattice) <<  std::endl;
            pcout << "Convergence: "<< ((relE < epsilon) ? "TRUE" : "FALSE") <<  std::endl;
            pcout << std::endl;
