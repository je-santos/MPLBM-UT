%% Experiment name and options (For decreasing rho2)
...
g_ads_f1_s2='0.4 ';
domain='spheres_equal_.dat ';
max_it='2000 ';
save_it='2000 ';
vtk_it='2000 ';
info_it='100 ';%displays info and checks for convergence
convergence='0.00001 ';
start_no='0 '; %no of first realization
run_no='2 '; %number of densities
...
nz1f2='1 '; 
nz2f2='120 ';
rho_no_fluid='0.06 '; %disolved density
cap_factor='1.0 ';
rho_fluid2_min = '1.5 ';
rho_fluid2_max = '2.0 ';
rho_fluid2_step = '0.05 ';
rho_fluid1 = '2.0 ';

%Writting Pressure
runs=str2num(run_no);
pressure=[];
pressure(1)=2.0;  %initial density
...
pres_str1=num2str(pressure(1));
pres_str1='2.0'; %initial density
pres_str22='2.0';
for i=1:runs-1
...
str_sc=['ibrun ./ShanChen_v4_2 ',path_output,omega1,omega2,nx,ny,nz,g,...
    force_f1,force_f2,g_ads_f1_s1,g_ads_f1_s2,domain,max_it,save_it,vtk_it,info_it,...
    convergence,start_no,run_no,nx1f1,nx2f1,ny1f1,ny2f1,nz1f1,nz2f1,nx1f2,nx2f2,ny1f2,ny2f2,nz1f2,nz2f2,...
    rho_no_fluid,cap_factor,rho_fluid2_min,rho_fluid2_max,rho_fluid2_step,rho_fluid1];
...
if create_batch==true
    ...
    fprintf(fileID_batch,'#SBATCH -N 5                # number of nodes requested \n');
    fprintf(fileID_batch,'#SBATCH -n 340               # total number of mpi tasks requested \n');
    fprintf(fileID_batch,'#SBATCH -p normal      # queue (partition) -- normal, development, etc. \n');
    fprintf(fileID_batch,'#SBATCH -t 12:00:00         # run time (hh:mm:ss) - 1.5 hours \n\n');
...
if send_batch==true
    system(['sbatch batchjob_',exp_name])
else
