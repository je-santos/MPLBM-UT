        ...
        for k = 1:length(neighbor_ids)
            ...
            if ~settled(cidx)
               % d = sqrt(sum((nodes(pidx,2:cols) - nodes(cidx,2:cols)).^2));
                num=size(segments,1);  %%% Modified to consider distances calculated along tortuous path instead of point-to-point
                    for count=1:num
                        if(segments(count,2)== pidx && segments(count,3)== cidx || segments(count,2)== cidx && segments(count,3)== pidx )
                                   d = segments(count,4);
                                   break
                        end
                    end
               %  [r,~] = find(segments(:,2)== pidx & segments(:,3)== cidx); %
