...
- `simulation`:
  ...
  - `convergence:` number indicating the convergence threshold. From testing, a value of 0.01 gives consistent results while maintaing reasonable run times.
  - `convergence iter:` Integer number of how often convergence should be checked. This should be approximately the same as `nx` (ie For `nx`=100, `convergence iter = 100`). You may have to further tweak this depending on your geometry. 
