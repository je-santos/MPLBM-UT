...
void writeGifs(MultiBlockLattice3D<T, DESCRIPTOR>& lattice_fluid1,
    ...
    MultiBlockLattice3D<T, DESCRIPTOR>& lattice_fluid2, plint runs, plint iT)
{
    ...
    ImageWriter<T> imageWriter("leeloo.map");
    imageWriter.writeScaledGif(createFileName("y_rho_f1_", 100000000 * runs + iT, 8),
        *computeDensity(lattice_fluid1, slice), imSize, imSize);
    //imageWriter.writeScaledGif(createFileName("rho_f2_", iT, 8),*computeDensity(lattice_fluid2, slice), imSize,imSize);
    //imageWriter.writeScaledGif(createFileName("velz_f1", iT, 8),*computeVelocityComponent(lattice_fluid1, slice, zcomponent), imSize,imSize);
}
...
void writeVTK(MultiBlockLattice3D<T, DESCRIPTOR>& lattice, plint runs, plint iter, plint nx, plint ny, plint nz) // std::string Dstring, std::string Vstring
{
    ...
    VtkImageOutput3D<double> vtkOut(createFileName("rho1_", 100000000 * runs + iter, 8), 1.);
...
int main(int argc, char* argv[])
{
    ...
    const plint iter_info = atoi(argv[13]);
    const plint iter_max = atoi(argv[13]);
    const T iter_conv = atof(argv[17]);
    const plint iter_gif = atoi(argv[14]);
    const plint iter_vtk = atoi(argv[15]);


    const plint startNum = atoi(argv[18]);
    const plint runnum = atoi(argv[19]);
    ...
    lattice_fluid2.periodicity().toggle(1, true);
    lattice_fluid1.periodicity().toggle(1, true);
    lattice_fluid2.periodicity().toggle(2, true);
    lattice_fluid1.periodicity().toggle(2, true);
